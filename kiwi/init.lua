...
Kiwi2D.push = require(LUX_PATH .. "libraries.push")
...
Kiwi2D.Components = {}       -- store the engine components --
Kiwi2D.engine = import 'Engine'
Kiwi2D.scene = import 'core.Scene'
Kiwi2D.object = import 'core.Object'


Kiwi2D.assets = import 'utils.AssetPool'
Kiwi2D.assets.addImage("logo", LUX_PATH .. "/assets/images/icon.png")
Kiwi2D.assets.addFont("fredoka", LUX_PATH .. "/assets/fonts/fredoka_regular.ttf")
...
function Kiwi2D.init(config)
    ...
    if not Kiwi2D.scene.gameScenes[conf.scene] then
    ...
    love.run = function()
        ...
        local fpsfont = Kiwi2D.assets.get(Kiwi2D.assets.AssetType.FONT, "fredoka", { fontsize = 18 })

        Kiwi2D.engine.onLoad:trigger()
        ...
        return function()
            if love.event then
                ...
                for name, a,b,c,d,e,f in love.event.poll() do
                    if name == "quit" then
                        Kiwi2D.engine.onQuit:trigger()
                        if not love.quit or not love.quit() then
                            return a or 0
                        end
                    elseif name == "resize" then
                        Kiwi2D.push.resize(a, b)
                        Kiwi2D.engine.onWindowResize:trigger(a, b)
                    end
            ...
            Kiwi2D.engine.onWindowStateChange:trigger(love.window.hasFocus())
    
            local fpsCap = isFocused and love._FPSCap or love._unfocusedFPSCap
            if love.timer then 
                elapsed = love.timer.step() * Kiwi2D.engine.timeScale
            end

            Kiwi2D.scene.update(elapsed)
            ...
            if love.graphics and love.graphics.isActive() then
                ...
                love.graphics.push("all")
                    love.graphics.setCanvas(mainCanvas)
                        ...
                        Kiwi2D.engine.onSceneDraw:trigger()
                        Kiwi2D.scene.draw()
                    love.graphics.setCanvas()
                love.graphics.pop()

                Kiwi2D.engine.onPreDraw:trigger()
                love.graphics.origin()
                love.graphics.clear(love.graphics.getBackgroundColor())
                ...
                Kiwi2D.engine.onPostDraw:trigger()
            end
